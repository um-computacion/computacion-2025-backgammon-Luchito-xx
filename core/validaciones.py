...
from .exceptions import (
    ...
    SalidaInvalidaError, BackgammonError
)
...
class Validaciones:
    ...
    def movimiento_valido(celdas:list, capturas, celda:int, salto:int, jugador:str):
        ...
        #Validar origen
        Validaciones._validar_origen(celdas, celda, jugador)
        ...
        # validar destino
        if (celdas[destino] and
            celdas[destino][0].get_jugador() != jugador and
            len(celdas[destino]) >= 2):
            raise CeldaBloqueadaError("Destino bloqueado por fichas enemigas")

        #Validar destino
        Validaciones._validar_destino(celdas, destino, jugador)

        return True

    @staticmethod
    def _validar_origen(celdas: list, celda: int, jugador: str) -> None:
        """
        Valida condiciones básicas del origen: rango, existencia y pertenencia.
        """
        if celda == -1:
    ...
    @staticmethod
    def _validar_destino(celdas: list, destino: int, jugador: str) -> None:
        """
        Valida el destino del movimiento.
        """
        if destino < 0 or destino > 23:
            # Dejar que validar_salida se encargue
            return
    ...
    def validar_salida(celdas: list, capturas: list, celda: int, salto: int, jugador: str):
        ...
        if Validaciones._tiene_fichas_fuera_de_casa(celdas, jugador, rango_casa):
            raise SalidaInvalidaError("No todas tus fichas están en casa")

        return True

    @staticmethod
    def _tiene_fichas_fuera_de_casa(celdas: list, jugador: str, rango_casa: range) -> bool:
        """Devuelve True si hay al menos una ficha del jugador fuera de su casa."""
        for i in range(24):
            if i not in rango_casa:
                for ficha in celdas[i]:
                    if ficha.get_jugador() == jugador:
    ...
    @staticmethod
    def _hay_fichas_mas_atras(celdas: list, celda: int, jugador: str) -> bool:
        """Devuelve True si hay fichas del jugador más atrás que (celda)."""
        if jugador == "X":
